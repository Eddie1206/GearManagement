...
{
    ColorfulWrite(str, ConsoleColor.Red);
}
...
while (true) {
    ...
    else if(intSys == 2)
    {
        ...
        {
            //电机配方系统在此测试
            ColorfulWrite("\n已进入电机配方系统，输入参数继续操作(指令 1 2 3 另需附带额外参数)", ConsoleColor.Blue);
            ColorfulWrite("0: 显示配方\n" +
                                    "1: 添加配方\targ[1]:型号名\n" +
                                    "2: 删除配方\targ[1]:配方ID\n" +
                                    "3: 生产\t\targ[1]:配方ID\targ[2]:数量", ConsoleColor.Blue);
            string? commandLine = Console.ReadLine();
            commandLine = commandLine.TrimEnd();
            string[] arg = commandLine.Split(" ");
			int op; 
            if (!int.TryParse(arg[0], out op))
				continue;
            Console.WriteLine("arg.Length: " + arg.Length);
            int id;
            if (op == -1)
            {
                break;
            ...
            switch (op)
            {
                ...
                case 1:
                {
                    ...
                    if (gearA.Length != quantityA.Length)
                    {
                        WarnMsg("输入有误");
                        continue;
                    }

                    int quantityIntCheck = 0;
                    for (int i = 0; i < gearA.Length; i++)
                    {
                        //int quantityInt;
                        if (!int.TryParse(quantityA[i], out int quantityInt))
                        {
                            quantityIntCheck++;
                            WarnMsg("输入有误");
                            break;
                ...
                case 2:
                    ...
                    if (!int.TryParse(arg[1], out id))
                    {
                        WarnMsg("输入有误");
                        continue;
                    }
                    mr.DeleteItem_outdated(id);
                    break;
                case 3:
                    if (arg.Length != 3)
                    {
                    ...
                    if (!int.TryParse(arg[2], out int num))
                    {
                        WarnMsg("输入有误");
                        continue;
                    }
